...
def ParallelULA(gamma, Niter, X0):
    ...
    for i in range(1, Niter):
        gradient = gradient_banana(X[i-1, :, :])
...
def mala_accept_reject(prop, v, gamma):
    d = prop.shape[0]
    log_proposal = multivariate_normal.logpdf((v-(prop+gamma*gradient_banana(prop))).T, np.zeros(d), 2*gamma*np.eye(d))-multivariate_normal.logpdf((prop - (v+gamma*gradient_banana(v))).T, np.zeros(d), 2*gamma*np.eye(d))
    log_acceptance = logpi_banana(prop) - logpi_banana(v) + log_proposal
...
def SMC_WFR(gamma, Niter, X0):
    ...
    for n in range(1, Niter):
        ...
        # MCMC move
        gradient_step = X[n-1, :, :] + gamma*gradient_banana(X[n-1, :, :])
...
#         H = np.median(pairwise_squared_distances)/(2*np.log(N))*np.eye(d) # -- median euristic
        ...
        logW = (1-np.exp(-gamma))*(logpi_banana(X[n, :, :])-np.log(weight_denominator))
        W[n, :] = rs.exp_and_normalise(logW)
    return X, W

def SMC_ULA(gamma, Niter, X0):
    d = X0.shape[1]
    N = X0.shape[0]
    X = np.zeros((Niter, d, N))
    X[0,:,:] = X0.T
    ...
    for n in range(1, Niter):
        ...
        logW = delta*(logpi_banana(X[n, :, :]) +0.5*np.sum(X[n, :, :]**2, axis = 0))
        W[n, :] = rs.exp_and_normalise(logW)
    return X, W

def SMC_MALA(gamma, Niter, X0):
    d = X0.shape[1]
    N = X0.shape[0]
    X = np.zeros((Niter, d, N))
    X[0,:,:] = X0.T
    ...
    for n in range(1, Niter):
        ...
        logW = delta*(logpi_banana(X[n-1, :, :]) +0.5*np.sum(X[n-1, :, :]**2, axis = 0)) - delta*np.exp(-gamma)*(logpi_banana(X[n, :, :]) +0.5*np.sum(X[n, :, :]**2, axis = 0))
        W[n, :] = rs.exp_and_normalise(logW)
    return X, W, accepted

### FR
def SMC_FR(gamma, Niter, X0):
    d = X0.shape[1]
    N = X0.shape[0]
    X = np.zeros((Niter, d, N))
    X[0,:,:] = X0.T
    ...
    for n in range(1, Niter):
        ...
        # reweight
        delta = l - lpast
        logW = delta*(0.5*np.sum(X[n, :, :]**2, axis = 0) + logpi_banana(X[n, :, :]))
        W[n, :] = rs.exp_and_normalise(logW)
    return X, W
...
def rwm_accept_reject(prop, v, l):
    log_acceptance = (1-l)*0.5*np.sum(v**2 - prop**2, axis = 0) + l*(logpi_banana(prop)-logpi_banana(v))
    accepted = np.log(np.random.uniform(size = v.shape[1])) <= log_acceptance
    output = np.copy(v)
    output[:, accepted] = prop[:, accepted]
    return output
